...
int main(int argc, char *argv[]) {
  ...
  while (getline(work_file, line)) {
    ...
    if (line.length() > 0 && (line[0] != 'F')) {
      ...
      // End of batch -- copy input and results
  ...
  if (status_bytes != sizeof(status_buffer) || (status_buffer[0] != 'R' && status_buffer[0] != 'r') || status_buffer[5] != '\n') {
  ...
  for (unsigned long batch = 0; batch != input_batches.size() && failure_cnt < MAX_FAILED_QUERIES; ++batch) {
    ...
    for (unsigned i = 0; i != result_batches[batch].size() && failure_cnt < MAX_FAILED_QUERIES; ++i) {
      ...
      if (!matched) {
        cerr << "Result mismatch for query " << query_no << ", expected: " << result_batches[batch][i] << ", actual: " << val
             << endl;
        ++failure_cnt;
      }
      /*if (matched)
